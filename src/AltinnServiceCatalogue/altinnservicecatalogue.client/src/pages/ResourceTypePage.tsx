...
export default function ResourceTypePage() {
  ...
  useEffect(() => { fetch(`/api/v1/${env}/resource/resourcelist?includeApps=true&includeAltinn2=true`).then((r) => r.ok ? r.json() as Promise<ServiceResource[]> : Promise.reject(new Error(String(r.status)))).then((all) => setResources(all.filter((r) => r.resourceType === resourceType))).catch((e) => setError(String(e))).finally(() => setLoading(false)); }, [env, resourceType]);
